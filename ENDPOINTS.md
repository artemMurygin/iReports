...
- Документы начисления зарплаты (`SalaryAccrual`, PRD 1 docs/payroll-closing-and-accrual, Фаза 1) — рождаются только закрытием периода (см. `close` выше), один документ на `(direction, period, employeeId)`; чтение, без гарда:
  ...
  - `POST /v1/service/accounting/salary_accruals/:id/lines/:lineId/unaccrue` — отменить начисление строки (без тела): движения `SALARY_ACCRUAL` и `ACCRUAL_ADJUSTMENT` строки УДАЛЯЮТСЯ с баланса (не сторно — начисление до выплаты считается черновиком расчёта), строка → `DRAFT` в той же транзакции, статус документа пересчитывается; непроведённая строка или документ `PAID` → `409`; ответ — карточка документа
...
- Баланс сотрудника (`BalanceTransaction`, PRD 2 docs/payroll-closing-and-accrual, Фаза 6) — лента движений денег по паре `(employeeId, direction)`, единственный источник истины об остатке (хранимого поля «остаток» нет; суммы — Int в целых рублях со знаком); движения ленты неизменяемы (нет `PATCH`/`DELETE` движения), движения начисления удаляются только `unaccrue` строки. Без гарда:
  - `GET /v1/service/accounting/balance/employee/:id?from&to&types` — остаток (`balance` = `SUM` всей ленты пары, от фильтров не зависит) и движения с фильтрами: `from`/`to` — ISO-даты по дате движения `occurredAt`, `types` — список типов через запятую (`?types=SALARY_ACCRUAL,ADVANCE`; перечень заложен целиком: `SALARY_ACCRUAL, ACCRUAL_ADJUSTMENT, ADVANCE, EXTRA_ADVANCE, BONUS, SICK_LEAVE, VACATION_PAY, PENALTY, ADJUSTMENT, MANUAL_REVERSAL, PAYOUT` — ручные типы и сторно создаются Фазой 7, `PAYOUT` — PRD 3). Ответ `{ employeeId, direction, balance, selectionTotal, transactions[] }`; `selectionTotal` — сумма выборки; движение несёт ссылки-идентификаторы (`accrualId`/`lineId`/`ruleId`/`reversedTransactionId`), признаки `erpSyncRequired`/`isReversed` и раскрытие начисления `accrualLine` — та же форма строки, что в карточке документа (резолвится по `lineId` на чтении). Сотрудник без движений — `balance: 0` и пустая лента; общий баланс по двум направлениям собирается фронтендом из двух ответов
  - `POST /v1/service/accounting/balance/employee/:id/transactions` — ручное движение руководителя (Фаза 7 PRD 2): `{ type, amount, occurredAt?, comment?, period?, createdBy, erpSyncRequired? }`, `type` из `ADVANCE | EXTRA_ADVANCE | BONUS | SICK_LEAVE | VACATION_PAY | PENALTY | ADJUSTMENT`. Знак определяется типом (`ADVANCE`/`EXTRA_ADVANCE`/`PENALTY` — расход, `BONUS`/`SICK_LEAVE`/`VACATION_PAY` — приход; `amount` в запросе — абсолютная величина > 0), для `ADJUSTMENT` знак задаётся явно (`amount` со знаком, ≠ 0). Комментарий обязателен для `PENALTY`/`ADJUSTMENT` (→ `400`). `occurredAt` — дата движения, разрешена задним числом, в т.ч. внутри закрытого месяца — снапшот и документы начисления при этом НЕ меняются; `createdAt` в ленте показывает, что запись создана позже. Лимитов на аванс нет — отрицательный остаток штатен, остаток не проверяется. `erpSyncRequired` в этой итерации только хранится и отдаётся (касса ERP — PRD 3). Ответ — созданное движение (`BalanceTransaction`)
  - `POST /v1/service/accounting/balance/transactions/:id/reverse` — сторно ручного движения (`{ comment, createdBy }`, комментарий обязателен → `400`): создаёт `MANUAL_REVERSAL` на точную противоположность исходной суммы со ссылкой `reversedTransactionId`; исходное движение остаётся в ленте с признаком `isReversed: true`. Единственный способ исправить ручное движение — `PATCH`/`DELETE` движения не существует. Сторнируются только ручные движения без документа ERP: движение начисления/выплата → `409` (начисление отменяется `unaccrue` строки), `erpSyncRequired: true` → `409` (удаление вместе с документом ERP — PRD 3), сторно сторно → `409`, повторное сторно → `409` (прямой повтор — проверкой по ленте, гонка — уникальным ограничением БД на `reversedTransactionId`). Ответ — созданное движение сторно
  - `GET /v1/service/accounting/balance/department/:id/:period` — сводка балансов по отделу за месяц (Фаза 7 PRD 2): состав сотрудников — ТЕКУЩИЙ отдел из справочника Bitrix24 на момент запроса (в движении отдел не хранится). Ответ `{ departmentId, direction, period, employees[], totals }`; строка сотрудника — `{ employeeId, employeeName, balance, accrued, advances, manual, accrualStatus }`: `balance` — `SUM` всей ленты пары (от периода не зависит), `accrued` — движения начисления (`SALARY_ACCRUAL`/`ACCRUAL_ADJUSTMENT`) с полем `period` запрошенного месяца, `advances` — `ADVANCE`/`EXTRA_ADVANCE` по дате движения внутри месяца, `manual` — остальные ручные типы и `MANUAL_REVERSAL` по дате внутри месяца (суммы со знаком, как в ленте; `PAYOUT` — только в `balance`), `accrualStatus` — статус документа начисления за период (`null` — документа нет); `totals` — сумма строк сотрудников
...
- `GET /v1/shop/accounting/balance/employee/:id?from&to&types` — баланс сотрудника направления `shop`: та же форма и фильтры, что у сервисного эквивалента (`{ employeeId, direction: 'shop', balance, selectionTotal, transactions[] }`); лента направления `shop` не пересекается с `service`
- `POST /v1/shop/accounting/balance/employee/:id/transactions`, `POST /v1/shop/accounting/balance/transactions/:id/reverse` — ручные движения и сторно направления `shop` (Фаза 7 PRD 2): те же контракты, правила знака/комментария и коды ошибок, что у сервисных эквивалентов; generic-по-`direction` команды `CreateBalanceTransactionCommand`/`ReverseBalanceTransactionCommand` через общий `CommandBus`
- `GET /v1/shop/accounting/balance/department/:id/:period` — сводка балансов по отделу направления `shop`: та же форма, что у сервисного эквивалента; собственный экземпляр generic-сервиса `GetDepartmentBalancesService` в `ShopAccountingModule`
