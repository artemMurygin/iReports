...
- Просмотр/редактирование зарплатных схем (страница списка/деталей, Фаза "Редактирование зарплатных схем") — направление схемы НЕ хранимое поле, а следствие того, что у строки `motivation_schemas` есть ≥1 правило `direction='service'`; строка с 0 такими правилами (все правила принадлежат `shop`-стороне той же схемы) для этих трёх эндпоинтов не существует:
  ...
  - `DELETE /v1/service/motivation-schema/:id` — удалить мотивационную схему направления `service` целиком (change `delete-motivation-schema`). `404` при тех же условиях, что и `GET .../motivation-schema/:id`. Удаляет все правила `direction='service'` строки; саму строку `motivation_schemas` — только если после этого у неё не осталось правил `direction='shop'` (иначе строка остаётся, чистится только `serviceName`, правила `shop` не трогаются). Необратимо, без soft-delete. `204 No Content`
  - `DELETE /v1/service/accounting/salary-rules/:ruleId` — удалить ОДНО зарплатное правило направления `service` вместе с его задачей, немедленно (change `add-task-rule-task-lifecycle`; не путать с удалением правила через `PATCH .../motivation-schema/:id`, где удаление — побочный эффект замены всего набора правил). `404`, если правила нет. Для `TaskCompletion`: задача ТЕКУЩЕГО периода удаляется безвозвратно (`DELETE`-аналог, `DeleteTaskCommand`), задачи прошлых периодов регулярного правила — мягко отменяются (`CancelTaskForRuleDeletionService`, история начислений сохраняется). Необратимо для правила, без soft-delete. `204 No Content`
  - `POST /v1/service/accounting/salary-rules/:ruleId/deactivate` — мягко деактивировать ОДНО зарплатное правило направления `service`: правило перестаёт участвовать в расчётах и пропадает из UI схемы (`GET .../motivation-schema/:id`, отчёты), но не удаляется физически и не задевает связанные задачи (`TaskCompletion`) — в отличие от `DELETE .../salary-rules/:ruleId` выше. `404`, если правила нет. Зеркало — `POST /v1/shop/accounting/salary-rules/:ruleId/deactivate` ниже. `204 No Content`
  - `POST /v1/service/accounting/salary-rules/:ruleId/activate` — вернуть ранее деактивированное правило направления `service` обратно в расчёт и в UI схемы. `404`, если правила нет. Зеркало — `POST /v1/shop/accounting/salary-rules/:ruleId/activate` ниже. `204 No Content`
...
- Документы начисления зарплаты (`SalaryAccrual`, PRD 1 docs/payroll-closing-and-accrual, Фаза 1) — рождаются только закрытием периода (см. `close` выше), один документ на `(direction, period, employeeId)`; чтение, без гарда:
  ...
  - `GET /v1/service/accounting/salary_accruals/:id` — карточка документа: то же + `lines: SalaryAccrualLine[]` — строка на каждое зарплатное правило снапшота (`ruleId, type, name, targetRole, salaryBasis, quantity, rate, amount, originalAmount, status, adjustmentComment, sources, comment, requiresManualInput`; `comment`/`requiresManualInput` — раздел 13 tasks.md add-task-based-salary-rule, первичный ручной ввод суммы строки `TaskCompletion`, см. `PATCH .../task-reward` ниже); `404`, если документ не найден или принадлежит другому направлению
  ...
  - `PATCH /v1/service/accounting/salary_accruals/:id/lines/:lineId/task-reward` — первичный ручной ввод суммы+комментария строки правила «за выполнение задачи» (`TaskCompletion`, раздел 13 tasks.md add-task-based-salary-rule, design.md Decision 5; `{ amount, comment }`, комментарий обязателен → `400` без него, БЕЗ `adjustedBy` — это первичный ввод, а не корректировка уже посчитанного значения): только для строки с `requiresManualInput === true` в статусе `DRAFT` — устанавливает действующую сумму `amount` и `comment`, сбрасывает `requiresManualInput`; строка, для которой ручной ввод не предусмотрен (не `TaskCompletion`, либо уже введено), или строка в `ACCRUED`/`PAID` → `409`; несуществующий документ/строка или документ другого направления → `404`; ответ — карточка документа
...
- `GET /v1/service/accounting/salary_role_types` — типы зарплатных правил сервиса (`PayPerHour`, `ServiceCompleted`, `OrderPayed`) с перечнем допустимых `targetRole` для каждого (Фаза 8)
...
- `POST /v1/auth/api-key/regenerate` — регенерирует персональный API-ключ текущего сотрудника
  (add-employee-api-key-auth, `openspec/changes/add-employee-api-key-auth`): инвалидирует прежнее
  значение (`BitrixEmployee.apiKeyHash`, `@unique`) и возвращает новое сырое значение в теле
  (`{ apiKey }`) — единственный момент, когда ключ виден в открытом виде после первичной генерации.
  Контроллер (`session/interface/http-controllers/regenerate-api-key.http-controller.ts`) защищён
  `SessionAuthGuard`+`CsrfGuard` (мутирующий запрос), доступен только по валидной Bitrix-сессии — не
  публичный; заголовок `X-Api-Key` для ЭТОГО эндпоинта не принимается (401 даже с валидным ключом —
  иначе скомпрометированный ключ мог бы сам себя перевыпускать), контроллер явно отклоняет запрос,
  если `SessionAuthGuard` аутентифицировал его по ключу, а не по сессии. Ключ, выданный этим
  эндпоинтом, аутентифицирует последующие запросы к ЛЮБЫМ ОСТАЛЬНЫМ защищённым маршрутам приложения
  заголовком `X-Api-Key` (ветка внутри `SessionAuthGuard`, минуя сессию/CSRF, с тем же RBAC, что и
  сессионный вход); ключ сотрудника с `isActive: false` не проходит аутентификацию.
...
- `PATCH /v1/service/sales/plan/order` — задать глобальный (общий для всех пользователей) порядок строк-категорий плана отдела (drag-and-drop в модалке редактирования, `docs/sales-plan-row-drag-and-drop-reorder`, Фаза 1); тело — `{ department, items: [{ category, sortOrder }] }`, ответ — изменённые строки `SalesPlanTemplate`; трогает только `SalesPlanTemplate.sortOrder` (upsert по естественному ключу `(department, category)`, для новой строки — с нулевыми `turnover`/`margin`), не строки `SalesPlan`; не блокируется закрытым расчётным периодом (сознательное решение Фазы 4, см. комментарий в `UpdateSalesPlanOrderHandler`)
- `GET /v1/service/sales/plan_template` — дефолтный шаблон плана направления `service` (оборот, маржа, процент роста, порядок строки) по отделам/категориям
...
- `PATCH /v1/shop/sales/plan/order` — задать глобальный (общий для всех пользователей) порядок строк-категорий плана отдела направления `shop` (drag-and-drop в модалке редактирования, `docs/sales-plan-row-drag-and-drop-reorder`, Фаза 4); тело/ответ — как у `PATCH /v1/service/sales/plan/order` выше; трогает только `SalesPlanTemplate.sortOrder` строк направления `shop`, не блокируется закрытым расчётным периодом (сознательное решение Фазы 4, см. комментарий в `UpdateShopSalesPlanOrderHandler`)
...
позиции периода отгрузки; `Fixed`/`FixedPercent`, без `FloatPercent`; необязательная категория).
Роли — `ONLINE_MANAGER`/`OFFLINE_MANAGER` (уровень отгрузки,
`MoySkladDemand`, используются `ProductSold`) и `ONLINE_PURCHASER`/
...
- `GET /v1/shop/accounting/salary_accruals/:id` — карточка документа начисления `shop` со строками (`lines[].comment`/`requiresManualInput` — раздел 18 tasks.md add-task-based-salary-rule, первичный ручной ввод суммы строки `TaskCompletion`, см. `PATCH .../task-reward` ниже); `404` для документа другого направления
- `POST /v1/shop/accounting/salary_accruals/:id/lines/:lineId/accrue`, `POST .../lines/:lineId/unaccrue`, `PATCH .../lines/:lineId` — проведение/отмена/корректировка строки документа `shop` (Фаза 6 PRD 2): те же контракты, статусы и коды ошибок, что у сервисных эквивалентов выше; тонкие HTTP-контроллеры поверх собственных, независимых команд `Accrue/Unaccrue/AdjustShopSalaryAccrualLineCommand`, движения ложатся на общий баланс сотрудника (Фаза 8b)
- `PATCH /v1/shop/accounting/salary_accruals/:id/lines/:lineId/task-reward` — первичный ручной ввод суммы+комментария строки правила «за выполнение задачи» (`TaskCompletion`, раздел 18 tasks.md add-task-based-salary-rule, зеркало сервисного эндпоинта выше, design.md Decision 5; `{ amount, comment }`, комментарий обязателен → `400` без него, БЕЗ `adjustedBy`): только для строки с `requiresManualInput === true` в статусе `DRAFT` — устанавливает действующую сумму `amount` и `comment`, сбрасывает `requiresManualInput`; строка, для которой ручной ввод не предусмотрен, или строка в `ACCRUED`/`PAID` → `409`; несуществующий документ/строка → `404`; ответ — карточка документа
...
- `GET /v1/shop/accounting/salary_role_types` — типы зарплатных правил магазина (`PayPerHour`,
  `ProductSold`, `UsedProductSold`) с перечнем допустимых `targetRole` для каждого;
  набор типов не пересекается с `GET /v1/service/accounting/salary_role_types` сервиса (кроме совпадающего по
  имени `PayPerHour` — это независимые типы с разными реестрами)
...
- Просмотр/редактирование зарплатных схем магазина (страница списка/деталей, зеркало одноимённых
  ...
  - `DELETE /v1/shop/accounting/motivation-schema/:id` — удалить мотивационную схему направления `shop`
    целиком (change `delete-motivation-schema`). `404` при тех же условиях, что и
    `GET .../motivation-schema/:id`. Удаляет все правила `direction='shop'` строки; саму строку
    `motivation_schemas` — только если после этого у неё не осталось правил `direction='service'`
    (иначе строка остаётся, чистится только `shopName`, правила `service` не трогаются). Необратимо, без
    soft-delete. `204 No Content`
  - `DELETE /v1/shop/accounting/salary-rules/:ruleId` — удалить ОДНО зарплатное правило направления
    `shop` вместе с его задачей, немедленно (change `add-task-rule-task-lifecycle`; зеркало
    `DELETE .../service/accounting/salary-rules/:ruleId` выше). `404`, если правила нет. Для
    `TaskCompletion`: задача текущего периода удаляется безвозвратно, задачи прошлых периодов
    регулярного правила — мягко отменяются. Необратимо для правила, без soft-delete. `204 No Content`
  - `POST /v1/shop/accounting/salary-rules/:ruleId/deactivate` — мягко деактивировать ОДНО зарплатное
    правило направления `shop`: правило перестаёт участвовать в расчётах и пропадает из UI схемы, но
    не удаляется физически и не задевает связанные задачи (зеркало
    `POST .../service/accounting/salary-rules/:ruleId/deactivate` выше). `404`, если правила нет.
    `204 No Content`
  - `POST /v1/shop/accounting/salary-rules/:ruleId/activate` — вернуть ранее деактивированное правило
    направления `shop` обратно в расчёт и в UI схемы (зеркало
    `POST .../service/accounting/salary-rules/:ruleId/activate` выше). `404`, если правила нет.
    `204 No Content`
