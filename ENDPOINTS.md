...
- Конфигурация кассы ERP направления (`ErpCashConfig`, PRD 3 docs/payroll-closing-and-accrual, Фаза 11) — одна запись на направление, редактируется администратором один раз, форма выплаты/ручного движения (Фаза 12) кассу/статью не выбирает. `GET`/`PUT` — generic по `direction` реализация (`GetErpCashConfigService`/`PutErpCashConfigHandler`), зеркальные эндпоинты `shop` (см. ниже) диспатчат ту же команду через общий `CommandBus`:
  - `GET /v1/service/accounting/erp_cash_config` — `{ direction: 'service', roappCashboxId, moySkladExpenseItemId: null, moySkladIncomeItemId: null, organizationId: null, updatedAt }`; до первого `PUT` — все поля `null`
  - `PUT /v1/service/accounting/erp_cash_config` — задать ID единственной кассы (Finance Account) RemOnline: `{ roappCashboxId }`. Подтверждено предварительным исследованием через MCP RoApp (2026-08-24): RemOnline поддерживает и создание, и удаление движения по кассе (`POST`/`DELETE /finance/accounts/{account_id}/transactions[/{id}]`) — блокер Фазы 11 снят. У RemOnline нет поля для сотрудника-получателя как агента транзакции (`client_id` — контрагент, не сотрудник компании) — назначение и ФИО сотрудника попадают в `description` текстом (Фаза 12)
...
- Конфигурация кассы ERP направления `shop` (`ErpCashConfig`, PRD 3 docs/payroll-closing-and-accrual, Фаза 11) — зеркалит `service.accounting.erpCashConfig` выше, `PUT` диспатчит ту же generic по `direction` команду (`PutErpCashConfigCommand`) через общий `CommandBus` (хендлер зарегистрирован в `AccountingModule` сервиса):
  - `GET /v1/shop/accounting/erp_cash_config` — `{ direction: 'shop', roappCashboxId: null, moySkladExpenseItemId, moySkladIncomeItemId, organizationId, updatedAt }`; до первого `PUT` — все поля `null`
  - `PUT /v1/shop/accounting/erp_cash_config` — задать статью расходов и юрлицо МойСклада: `{ moySkladExpenseItemId, organizationId, moySkladIncomeItemId? }`. Подтверждено предварительным исследованием через MCP moysklad (2026-08-24): МойСклад поддерживает и создание, и удаление расходного/приходного ордера (`POST`/`DELETE /entity/{cashout,cashin}[/{id}]`) — блокер Фазы 11 снят. `moySkladIncomeItemId` — задел на будущее, не используется адаптером: схема `CashIn` не содержит поля вроде статьи доходов вообще (в отличие от `expenseItem` у `CashOut`), поле сохраняется на случай появления такого механизма позже. `agent` ордера (обязательное поле и у `cashout`, и у `cashin`) документирован МойСкладом как «метаданные контрагента или юрлица» — без явной поддержки прямой ссылки на сотрудника (`Employee`); как адаптер (Фаза 12) будет резолвить сотрудника в агента ордера — TODO в `application/ports/erp-cash-document.port.ts` домена `shop`, требует проверки на стенде
