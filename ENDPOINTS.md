...
- Просмотр/редактирование зарплатных схем (страница списка/деталей, Фаза "Редактирование зарплатных схем") — направление схемы НЕ хранимое поле, а следствие того, что у строки `motivation_schemas` есть ≥1 правило `direction='service'`; строка с 0 такими правилами (все правила принадлежат `shop`-стороне той же схемы) для этих трёх эндпоинтов не существует:
  ...
  - Правило типа `TaskCompleted` в `rules` (`POST`/`PATCH` выше; change `salary-rule-bitrix-task`) — только для схемы, нацеленной на сотрудника (`404`/`409`-конфликт `TaskRuleRequiresEmployeeTargetException` для схемы отдела); контракт: `{ description, period, isRecurring, dueDate, rewardAmount }` — `dueDate` обязан лежать в границах `period` (`YYYY-MM`), `rewardAmount` — только фиксированная сумма (вариант `FloatPercent` для этого типа правила удалён, BREAKING). При сохранении `CreateSalaryRuleHandler` синхронно создаёт задачу «Зарплатная задача» в Bitrix24 (`BitrixTasksService.createTask`, ответственный = сотрудник схемы) и пишет её `bitrixTaskId` в `bitrixTaskIds` правила; ошибка Bitrix24 откатывает сохранение схемы целиком, ошибка записи в БД после успешного создания задачи компенсируется синхронным `deleteTask`. Текст задачи не хранится в iReports — только `bitrixTaskIds`/`period`. Удаление правила/схемы удаляет связанные задачи в Bitrix24 первым шагом; ошибка удаления в Bitrix24 блокирует удаление правила (`TaskRuleBitrixDeletionFailedException`)
- Расчётный период (`AccountingPeriod`, Фаза 6, дополнено Фазой 13.5 и разделено по направлениям Фазой 3 — см. docs/payroll/plan-payroll-calculation.md и docs/service-shop-route-split) — сервис и магазин закрываются НЕЗАВИСИМЫМИ эндпоинтами без `:direction` в пути (свой `AccountingPeriod`/снапшот на каждое направление): закрытие `service` не трогает открытый период/снапшот `shop`, и наоборот. `close` обслуживают два независимых класса-хендлера (`CloseAccountingPeriodHandler` здесь и `CloseShopAccountingPeriodHandler` в `domains/shop/modules/accounting`, без общего кода/кросс-доменных импортов); `get`/`reopen`/`recalculate` — общие, generic-по-`direction` классы, переиспользуемые обоими доменами через один `CommandBus`. Эндпоинты без гарда (см. «неблокирующие вопросы» PRD, то же решение, что и у `sales`):
  ...
  - `GET /v1/service/accounting/period/:period/close-preview` — сводка окна подтверждения закрытия (`ClosePeriodPreviewResponse`: `employeesCount`, `dismissedEmployeesCount`, `totalAmount`, `unapprovedPlanRows`, `employeesWithoutHours` — сотрудники с правилом `PayPerHour` без записи часов за месяц, `unclosedTaskRules` — правила-задачи месяца, чьи задачи Bitrix24 ещё не «Закрыта» (change salary-rule-bitrix-task); всегда `[]` для направления `shop` — `TaskCompleted` там ещё не читает Bitrix24); считается тем же калькулятором строк снапшота, что и `close`, поэтому совпадает с результатом реального закрытия; ничего не пишет и ERP не синхронизирует
...
- Выполнение задачи сотрудником (`TaskCompletion`, Фаза 8, см. docs/payroll/plan-payroll-calculation.md) — временный внутренний двухступенчатый воркфлоу подтверждения без интеграции с Bitrix24 Tasks. **Выведен из эксплуатации** (change `salary-rule-bitrix-task`, design.md Decision 10, шаг 1): расчёт правила `TaskCompleted` теперь читает статус и расчётный месяц напрямую из задачи Bitrix24 (см. новую группу ниже), записи `TaskCompletion` в расчёте больше не участвуют. `create`/`confirm`/`reject`/`delete` отвечают `410 Gone` (маршруты оставлены зарегистрированными ради внятной ошибки клиентам ещё не обновлённого UI, а не ради функциональности); `list`/`get` — по-прежнему рабочие, временно, для аудита уже существующих записей. Вся группа (эндпоинты, доменный код, Prisma-модель/таблица `task_completions`) удаляется полностью одним шагом в задаче 11.2, после ручного runbook-шага 11.1 — не раньше. Эндпоинты без гарда, как и остальной `accounting`. `TaskCompletion.direction` (Фаза 13, дефолт `'service'`) — эти эндпоинты всегда пишут/читают `direction: 'service'`; направление `shop` пишет/читает ту же таблицу (`direction: 'shop'`) через собственный, независимый CQRS-вход `POST/GET /v1/shop/accounting/task_completions*` (Фаза 13.5, см. ниже) — та же пара Zod-контрактов (`createTaskCompletionRequestSchema`/`confirmTaskCompletionRequestSchema`/…) переиспользует HTTP-DTO, а не бизнес-логика; отключение `service`-стороны в этой задаче `shop`-эндпоинтов не затрагивает:
  - `POST /v1/service/accounting/task_completions` — **410 Gone**. Было: сотрудник отмечает задачу выполненной (`{ employeeId, period, description, createdBy }`), сразу в статусе `PENDING_CONFIRMATION`
  - `POST /v1/service/accounting/task_completions/:id/confirm` — **410 Gone**. Было: руководитель подтверждает (`{ confirmedBy }`) → только такие записи участвовали в расчёте `TaskCompleted.calculate()`
  - `POST /v1/service/accounting/task_completions/:id/reject` — **410 Gone**. Было: руководитель отклоняет (`{ confirmedBy }`)
  - `DELETE /v1/service/accounting/task_completions/:id` — **410 Gone**. Было: удалить запись
  - `GET /v1/service/accounting/task_completions?period&employeeId` — рабочий, временно (до задачи 11.2): записи за период (все сотрудники) или одна запись, если указан `employeeId`
- Правило-задача `TaskCompleted`, привязанная к Bitrix24 (change `salary-rule-bitrix-task`, заменяет `TaskCompletion` выше как источник расчёта) — постановка и приёмка идут в самой задаче Bitrix24 (создаётся/удаляется автоматически при сохранении/удалении правила через `POST`/`PATCH /v1/service/motivation-schema*` и `DELETE`-путь схемы, отдельного CRUD-эндпоинта над самой задачей нет), iReports только читает статус/расчётный месяц (пакетно, `BitrixTasksService.getTasksBatch`) и хранит ручной ввод фактической суммы по закрытой задаче:
  - `PUT /v1/service/accounting/task_rules/actual_amount` — указать/изменить фактическую сумму по закрытой задаче правила (`{ ruleId, period, actualAmount }`, `0 <= actualAmount <= сумма правила`; `ruleId` — часть тела, не пути). Только для правила-задачи в статусе «Закрыта» и только для открытого расчётного периода (`409`, если задача не закрыта или период закрыт); `204` при успехе
