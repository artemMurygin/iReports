...
import { computeFunnel } from "@/lib/computeStats"
    ...
    <Card className="flex-1 shadow-sm flex flex-col min-h-0">
      <CardHeader className="pb-3 border-b border-gray-100 shrink-0">
        ...
        <CardDescription className="text-sm text-gray-500">
          Сводка по текущей воронке
        </CardDescription>
      </CardHeader>
      <CardContent className="p-0 overflow-y-auto min-h-0">
        {data.length === 0 && (
          <p className="text-sm text-gray-400 py-6 text-center">Нет данных</p>
        )}
        {data.map((item, idx) => {
          ...
          return (
            ...
            >
              ...
              <span className="flex-1 text-sm font-medium" style={{ color: nameColor, fontFamily: "Inter, sans-serif" }}>
                {item.name}
              </span>
              <span className="text-sm font-bold w-10 text-right" style={{ color: nameColor, fontFamily: "Inter, sans-serif" }}>
                {item.count}
              </span>
              <span className="text-sm text-gray-500 w-20 text-right" style={{ fontFamily: "Inter, sans-serif" }}>
