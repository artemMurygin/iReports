...
import { EMPTY_DEVICE_TYPE } from "@/lib/computeStats"
import { type DashboardFilters } from "@/types/filters"
import { type ApiEmployee, type ApiEnumValue } from "@/types/deal"
...
export function FilterBar({ filters, employees, sources, deviceTypes, stages, loading, onChange, onReset }: FilterBarProps) {
        ...
        <div className="flex items-center gap-4 px-6 py-3 bg-white border-b border-gray-200 shrink-0">
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar
                ...
                onSelect={(range) => {
                  const from = range?.from ? new Date(range.from.setHours(0, 0, 0, 0)) : range?.from
                  const to = range?.to ? new Date(range.to.setHours(23, 59, 59, 999)) : range?.to
                  onChange({ ...filters, dateRange: range ? { from, to } : range })
