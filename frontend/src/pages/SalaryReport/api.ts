...
export const salaryApi = {
    ...
    getCategories: (direction: Direction) =>
        api.get<CategoryNode[]>('/salary/categories', {
            params: { direction },
        }),
    ...
    updateRule: (id: number, data: Partial<SalaryRule>) => api.patch<SalaryRule>(`/salary-rules/${id}`, data),
    ...
    bulkCreatePlanTargets: (items: Array<Partial<PlanFactRow>>) => api.post('/plan-targets', { items }),
    updatePlanTarget: (id: number, data: Partial<PlanFactRow>) => api.patch(`/plan-targets/${id}`, data),
    deletePlanTarget: (id: number) => api.delete(`/plan-targets/${id}`),

    getWorkSchedule: (employeeId: number, period: string) =>
        api.get<WorkShift[]>('/work-schedule', {
            params: { employeeId, period },
        }),
    bulkUpsertShifts: (shifts: Array<Partial<WorkShift>>) => api.post('/work-schedule/bulk', { shifts }),
    updateShift: (id: number, data: Partial<WorkShift>) => api.patch(`/work-shifts/${id}`, data),

    getAdjustments: (employeeId: number, period: string) =>
        api.get<SalaryAdjustment[]>('/salary-adjustments', {
            params: { employeeId, period },
        }),
    ...
    getSalaryReport: (employeeId: number, period: string) =>
        api.get<SalaryReportData>('/salaryReport', {
            params: { employeeId, period },
        }),
    closeSalaryReport: (employeeId: number, period: string) =>
        api.post<SalaryReportData>('/salaryReport/close', {
            employeeId,
            period,
        }),
}
