 ...
 * form's "text input, parse on submit" convention (see `ruleFormSchema.ts`'s `parseNumber`) —
 * internally converts to/from the `Slider` atom's numeric API, clamping/defaulting an unparsable
 * string to `min` so the slider always has a valid position even before the user picks a value.
 */
export function PercentSliderField({ label, value, onValueChange, min = 1, max = 100, className }: PercentSliderFieldProps) {
            ...
            <div className="flex w-full flex-col gap-[9px]">
                <div className="flex w-full items-center justify-between gap-2.5">
                    ...
                    <span className="flex items-center gap-0.5 rounded-[6px] bg-brand-soft px-2 py-[3px]">
                        <span className="font-ui text-[13px] font-bold text-ink">{value.trim() === '' ? min : value}</span>
