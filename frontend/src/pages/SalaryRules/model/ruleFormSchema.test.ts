...
import { createRuleDraft, defaultBorders, type BorderDraft, type RuleDraft } from './ruleDraft.ts'
...
describe('resolveRuleDraft — PayPerHour', () => {
    it('succeeds with a valid rate', () => {
        ...
        if (result.success) {
            expect(result.data).toEqual({
                ...
                targetRole: 'ENGINEER',
                config: { price: 450, bonus: undefined },
            })
        }
    })
    ...
    it('rounds an optional integer bonus and accepts a comma decimal rate', () => {
        const result = resolveRuleDraft(baseDraft({ type: 'PayPerHour', price: '450,5', bonus: '1000' }))
        expect(result.success).toBe(true)
        if (result.success && result.data.type === 'PayPerHour') {
            expect(result.data.config.price).toBeCloseTo(450.5)
            expect(result.data.config.bonus).toBe(1000)
        }
    })
})
...
describe('resolveRuleDraft — ServiceCompleted award variants', () => {
    ...
    it('ServicePercent requires a percent', () => {
        const result = resolveRuleDraft(baseDraft({ type: 'ServiceCompleted', awardKind: 'ServicePercent', percent: '' }))
        expect(result.success).toBe(false)
...
describe('resolveRuleDraft — OrderPayed award variants', () => {
    ...
    it('FloatPercent fails when there are 4 percentBorders', () => {
        const fourBorders: BorderDraft[] = [...defaultBorders(), { name: 'Экстра', fromPlanPercent: '150', multiplier: '2', mode: 'FIX' }]
        const result = resolveRuleDraft(
            baseDraft({
                type: 'OrderPayed',
                awardKind: 'FloatPercent',
...
describe('resolveRuleDraft — TaskCompleted award variants', () => {
    ...
    it('FloatPercent requires basePrice and exactly 3 percentBorders, with no salaryBasis field', () => {
        const missingPrice = resolveRuleDraft(
            baseDraft({ type: 'TaskCompleted', awardKind: 'FloatPercent', basePrice: '', percentBorders: defaultBorders() }),
        )
