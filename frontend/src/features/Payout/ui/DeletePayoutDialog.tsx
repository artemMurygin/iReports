...
import { useDeletePayout } from '../model/usePayoutMutations.ts'

const ERP_SYSTEM_LABEL = { ROAPP: 'RemOnline', MOY_SKLAD: 'МойСклад' } as const
...
/**
 * Pencil `LqeWQ`/`bXM39` (десктоп) / `gvu2K` (мобильный) — «Удалить выплату N ₽ от ДД.ММ.ГГГГ?»
 * (Фаза 15 docs/payroll-closing-and-accrual, P3.3): будет удалён расходный документ ERP и
 * движение на балансе, документ начисления вернётся в «Ожидает выплаты» (сервер применяет это
 * ко всем затронутым документам направления — `DELETE .../payout/:id` не хранит обратную связь
 * «выплата → закрытые документы», см. заметку проверки Фазы 12). При отказе ERP — inline-ошибка
 * с текстом сервера и «Ничего не изменено», кнопка переключается в «Повторить» (тот же приём,
 * что `DeleteTransactionDialog`/`PayoutDrawer`).
 */
