...
const COLUMNS = 'grid-cols-[36px_minmax(220px,1fr)_170px_150px_100px_110px_150px_120px]'
 ...
 * Колонка «Действия» («Начислить»/«Отменить»/«Корректировать») — Фаза 9 (мутации),
 * в этой фазе документ только читается. Строка раскрывается аккордеоном в источники
 * (`AccrualLineSources`).
 */
...
function AccrualLinesTable({ lines, isLineExpanded, onToggleLine, footerNote, footerTotal, className }: AccrualLinesTableProps) {
    return (
        <div
                        ...
                        return (
                            ...
                            >
                                <button
                                    ...
                                    className={cn(
                                        'grid w-full items-center gap-2 px-3 py-3 text-left transition-colors hover:bg-canvas',
                                        COLUMNS,
                                    )}
                                >
                                    <ChevronDown
