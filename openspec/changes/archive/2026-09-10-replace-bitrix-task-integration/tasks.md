...
- [x] 1.3 Удалить модель `SalaryTask`/таблицу `salary_tasks` из Prisma-схемы (`prisma/schema/
...
- [x] 16.2 Прогнать `npm run test:e2e` (backend) — зафиксировать зелёный прогон, включая e2e из
      группы 5.
- [x] 16.3 Ручной сквозной сценарий (или e2e, если получится автоматизировать без /v1/tasks-мока):
