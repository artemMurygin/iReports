...
- [x] 21.1 Запустить бэкенд и фронтенд локально (`npm run start:dev` / `npm run start`),
  ...
  **Выполнено**: backend (`PORT=3001 npm run start:dev`, worktree `.env`) и frontend
  (`VITE_API_URL=http://localhost:3001 npx vite --port 5175`) подняты в фоне (порты 3000/5173
  заняты бэкендом/фронтендом соседнего worktree, не трогались), Playwright открыл
  `/goods-turnover-report`. Подтверждено вживую:
  - Filter Row рендерится полностью: `WarehouseSelect` (реальный склад `id=38107` из справочника
    задачи 14), `CategoryTreeSelect` (реальное дерево из ~200 категорий RemOnline, поиск и выбор
    категории работают — клик по «04 | Запчасти iPhone» меняет триггер без лишнего сетевого
    запроса, подтверждено `browser_network_requests`), `PeriodPicker`, `PeriodStatusBadge`.
  - Переключение периода клавишами «Предыдущий/Следующий месяц» инициирует ровно один новый
    `GET /v1/service/warehouse/goods-turnover-report/:period` на каждый месяц (проверено
    `browser_network_requests` — 2026-09→08→07→06, без дублей/зацикливания), справочники
    категорий/складов не перезапрашиваются.
  - Закрытый период (`service`/`2026-06`, реально существует в dev-БД, `status=CLOSED`) —
    бейдж «Период закрыт · данные зафиксированы», ни одной кнопки закрытия/пересчёта рядом
    (design.md D7); открытый период (`2026-09`) — «Открыт · пересчёт каждый час».
  - Адаптивность 390×844 — `document.documentElement.scrollWidth === window.innerWidth` (без
    горизонтального overflow), тот же результат, что и в задаче 20.3.
  **Отклонение от буквального «отчёт строится по реальным данным»**: во всех проверенных
  периодах (2026-06…2026-09) `GET .../goods-turnover-report/:period` вернул `{lines: []}` —
  показывается состояние «Отчёт ещё не пересчитан» (валидный экран из задачи 19.3), НЕ ошибка.
  Причина подтверждена, не выдумана: `POST rm.murygin.tech/getGoodsFlowReport` (и корень `/` того
  же хоста) стабильно отдаёт `502 Bad Gateway` из этой песочницы (тот же сетевой блокер, что
  задокументирован задачами 1.2/4.3/9.3 — `curl` с `--max-time 10` перепроверен заново перед этой
  задачей). Часовой крон реально включён в этом окружении (`ENABLE_CRON=true` в `backend/.env`,
  не предполагалось, а проверено) и физически пытался пересчитать оба периода — лог `npm run test`
  того же прогона содержит `[BuildGoodsTurnoverReportService] Не удалось получить данные... — 502
  Bad Gateway`, т.е. пайплайн построения отчёта работает и вызывается правильно, просто upstream
  ERP-эндпоинт недоступен из песочницы (не полночный `EVERY_HOUR`-тик — за время ручной проверки
  реальный часовой тик не наступил, а инициировать пересчёт вручную нет отдельного эндпоинта —
  так и предполагает архитектура, крон/событие закрытия — единственные источники). Прямая ручная
  вставка тестовой строки в `goods_turnover_report_lines` через `psql` для проверки рендера
  таблицы с реальным коэффициентом заблокирована классификатором авто-режима песочницы (destructive
  SQL) — не обойдено. Рендер таблицы с реальными строками и коэффициентом уже визуально
  подтверждён отдельно задачей 18.5 (временный `/goods-turnover-preview` со снятыми со скриншота
  тестовыми данными) — здесь же подтверждён весь путь ВОКРУГ таблицы (фильтры, статус периода,
  сеть, адаптивность) на реальном бэкенде/БД/справочниках. Оба процесса (`nest`/`vite` этого
  worktree на портах 3001/5175) остановлены `pkill` по завершении проверки.
