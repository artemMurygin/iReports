...
import { RoappService } from '../../domains/service/integrations/roapp/roapp.service';
import { CustomApiRoappService } from '../../domains/service/integrations/custom-api-roapp/custom-api-roapp.service';
import { Service } from '../../domains/service/integrations/roapp/schemas/services.schema';
import { Category } from '../../domains/service/integrations/roapp/schemas/serviceCatalog.schema';
...
export class PriceMonitoringService {
    ...
    async updateServicePricesInRoapp(items: UpdateServicePricesInRoappItem[]) {
        ...
        const rows = items.flatMap((item) => {
            ...
            if (!service) {
                console.warn(
                    `Roapp service ${item.id} not found, skipping row`,
                );
                return [];
            }
  ...
  ${JSON.stringify(names)}`;
    ...
    private buildMoySkladUpdates(items: MatchedProduct[]) {
        ...
        return items
            ...
            .map((item) => ({
                ...
                buyPrice: {
                    value:
                        Math.round(parseFloat(String(item.price)) * 100) || 0,
                    currency: {
                        meta: {
                            href: currencyHref,
                            type: 'currency',
    ...
    private async writeResultsToSheet(
        ...
        const updates = matched
            .filter(
                (item) =>
                    item.price != null && idToSheetRow.has(item.externalId),
            )
            .map((item) => ({
