...
// связано. Без этого штампа ЛЮБОЙ переход статуса задачи (в т.ч. в
// «Выполнена»/«Закрыта успешно», см.
// requirement-строка-правила-за-выполнение-задачи-появляется-сразу-и-растёт-по-статусу-задачи,
// task-completion-progressive-visibility) не инвалидирует уже посчитанный
// кэш открытого периода — строка правила остаётся со старыми
// fact/prognose до случайного совпадения с одним из трёх других
// источников. Инвалидация зависит от того, что TaskRepository.update()
// действительно продвигает Task.updatedAt на каждой записи (Prisma
// @updatedAt автоматически, без явной перезаписи в data — иначе штамп
// замирает на старом значении и кэш никогда не считается устаревшим).
