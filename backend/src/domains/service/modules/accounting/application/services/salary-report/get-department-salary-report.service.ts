...
import {
    SalaryRule,
    type DepartmentPercentSalaryConfig,
    type DepartmentPlanBonusSalaryConfig,
    type DepartmentTurnoverBonusSalaryConfig,
...
export class GetDepartmentSalaryReportService {
    constructor(
        ...
        // BuildServiceCalculationContextService, здесь резолвится один раз на весь отдел
    ...
    private async buildOpenContributions(
        ...
        // отдела, уже развёрнутое ResolveEmployeeSalaryRulesService на
        // каждого) собираются в один список ДО цикла.
        ...
        // Штамп свежести на каждого сотрудника отдельно (см. WHY у
        // taskCompletionFreshnessStamp), а не общий на весь отдел: иначе
        // завершение чьей-то одной задачи инвалидировало бы кэш всех
        // сотрудников отдела разом.
        const taskCompletionTasksByRuleId = new Map(
            taskCompletionTasks.map((ref) => [ref.ruleId, ref]),
        );
        ...
        // "не должно быть N+1", что и у taskCompletionStatuses выше), по union
        // category/(warehouseId, category) DepartmentPercent/DepartmentPlanBonus/
        // DepartmentTurnoverBonus-правил ВСЕХ сотрудников отдела (allRules, уже собранный выше) — а
        // не через BuildServiceCalculationContextService (этот отчёт строит erpData/SalesPerformance
        // сам, батчем на отдел, см. шапку файла). Обе карты не зависят от employeeId, поэтому одни и
        ...
        for (const employee of employees) {
            ...
            // Задача регулярного TaskCompletion-правила на новый период
            // заводится только TaskCompletionAutoCreationCron (1 числа в
            // 10:00, см. accounting.module.ts) — см. WHY в
            // GetEmployeeSalaryReportService: ленивое достраивание отсюда
            // убрано, чтобы открытие отчёта за прошедший период не
            // насоздавало задним числом задачи за все такие периоды.
    ...
    // BuildServiceCalculationContextService.resolveDepartmentSalesPerformance, но по union category
    // ВСЕХ схем отдела (allRules) разом, а не по схеме одного сотрудника: этот отчёт уже знает
    // departmentId параметром execute(), поэтому карта строится безусловно (в отличие от
    // BuildServiceCalculationContextService, где departmentId может быть null при отсутствии отдела
    // у сотрудника — здесь такого случая нет, отчёт всегда вызывается для конкретного отдела).
    ...
    // Implements FR4 of add-department-head-salary-rules — зеркало
    // BuildServiceCalculationContextService.resolveTurnoverPerformance, по union
    ...
    // Тот же мост string → RoApp category id, что и в
    // BuildServiceCalculationContextService.toRoappCategoryId (см. подробный комментарий там,
    // "PRE-EXISTING CROSS-GROUP TYPE GAP") — TurnoverPerformanceReaderPort.findForScope принимает
    // категорию уже распарсенным числом.
