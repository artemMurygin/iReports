...
// Массовая выплата (PRD 3, «День выплаты»): результат — перечень
// PAID/NEEDS_CONFIRMATION/FAILED, один упавший сотрудник не прерывает
// остальных. CreatePayoutHandler зарегистрирован на реальном CommandBus
// (через CqrsModule), чтобы проверить именно диспатч, а не подмену.
describe('CreatePayoutBatchHandler', () => {
    ...
    const build = async (erpPort: ErpCashDocumentPort) => {
        ...
        const batchHandler = moduleRef.get(CreatePayoutBatchHandler);
    ...
    it('частичный успех: PAID / NEEDS_CONFIRMATION (нулевой остаток) / FAILED (ошибка ERP)', async () => {
        const erpPort: ErpCashDocumentPort = {
            create: (params: CreateErpCashDocumentParams) => {
                if (params.employeeId === 3) {
                    return Promise.reject(new Error('RemOnline недоступен'));
                }
        ...
        expect(byEmployee.get(3)?.message).toContain('RemOnline недоступен');
