...
// Выполнение задачи сотрудником — источник данных для
// TaskCompletedEntity.calculate() (Фаза 8, см.
// docs/payroll/plan-payroll-calculation.md, раздел "Принятые решения по
// открытым вопросам", п. 2).
//
...
export interface TaskCompletionCreateProps {
    employeeId: number;
    period: string;
    description: string;
    createdBy: number;
...
export class TaskCompletion extends Entity<TaskCompletionProps> {
    declare protected readonly _id: AggregateID;

    static create(props: TaskCompletionCreateProps): TaskCompletion {
    ...
    get status(): TaskCompletionStatus {
    ...
    private transition(
        ...
        if (this.props.status !== 'PENDING_CONFIRMATION') {
            throw new TaskCompletionInvalidStatusTransitionException(
                this.props.status,
            );
        }
