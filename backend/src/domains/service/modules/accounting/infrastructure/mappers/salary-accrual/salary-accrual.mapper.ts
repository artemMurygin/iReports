...
// фолбэком, что и target.name в MotivationSchemaMapper.toListItemResponse.
...
export class SalaryAccrualMapper {
    ...
    static unknownEmployeeInfo(employeeId: number): SalaryAccrualEmployeeInfo {
    ...
    // Та же форма строки используется раскрытием движения начисления в
    // ленте баланса (GetEmployeeBalanceService) — PRD 2: «начисление в
    // ленте раскрывается до правила и источников, идентичных строке
    // документа начисления».
    private toLineResponse(line: SalaryAccrualLine): SalaryAccrualLineContract {
        return {
            id: line.id,
            ruleId: line.ruleId,
            type: line.type,
            name: line.name,
            // Сущность хранит роль как string (direction-агностична),
            // контракт — как перечисление ролей обоих направлений; строка
            // попала в документ из RuleBreakdownLine правила, чья роль уже
            // прошла targetRoleSchema при создании правила.
