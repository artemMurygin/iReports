...
// Собирает freshness-штамп ленивого кэша расчёта зарплаты по ключу (период,
// сотрудник, версия мотивационной схемы) + сравнение штампа последней
// успешной синхронизации домена. Инвалидация обязана срабатывать на ТРИ
// события: завершение синхронизации ERP, правку схемы/правила, правку или
// утверждение плана продаж — все три сворачиваются в одну строку сравнения
// (см. buildStamp), вычисляемую заново при каждом чтении отчёта (см.
// зеркальный класс сервиса за полным обоснованием этого приёма).
//
