...
import { resolveShopEmployees } from '@/domains/shop/modules/accounting/application/services/salary-accrual/list-salary-accruals.service';
...
import { CreateShopPayoutBatchCommand } from './create-payout-batch.command';
...
// как FAILED.
//
// resolveShopEmployees/ShopSalaryAccrualMapper.unknownEmployeeInfo — собственные независимые
    ...
    async execute(
        ...
        for (const employeeId of command.employeeIds) {
            const employeeName = (
                ...
                ShopSalaryAccrualMapper.unknownEmployeeInfo(employeeId)
            ).name;
            const balance =
