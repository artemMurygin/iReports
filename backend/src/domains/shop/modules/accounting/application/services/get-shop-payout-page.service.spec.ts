...
import { GetShopPayoutPageService } from './get-shop-payout-page.service';

// Страница выплаты направления shop (PRD 3, «Контракты») — зеркалит
// get-payout-page.service.spec.ts направления service: payoutStatus и срезы
// accrued/advances/manual/paid — только по своему направлению; итог — сумма
// строк.
describe('GetShopPayoutPageService', () => {
    ...
    it('строка сотрудника: accrued/advances/manual только своего направления, balance общий, payoutStatus по остатку/paid', async () => {
        ...
        // Ручной аванс этого месяца, направление shop.
        await transactionRepo.insertMany([
            withRequestContext(() =>
        ...
        // Движение того же периода, но направления service — не должно
        // попасть в срезы страницы shop.
        await transactionRepo.insertMany([
            withRequestContext(() =>
                BalanceTransaction.createManual({
                    employeeId: 42,
        ...
        // Частичная выплата этого месяца, направление shop.
        await transactionRepo.insertMany([
            withRequestContext(() =>
        ...
        const service = new GetShopPayoutPageService(
            transactionRepo,
            accrualRepo,
            fakeDirectoryRepo,
        );
        ...
        // balance — SUM ВСЕЙ ленты, включая движение направления service:
