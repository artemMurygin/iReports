...
//
// spec: shop/accounting#requirement-мотивационная-схема-как-набор-зарплатных-правил-из-двух-источников
//
// Раньше расчёт звал ShopMotivationSchemaRepositoryPort.findByEmployee
// напрямую и видел только личную половину, поэтому сотрудник, мотивация
// которого заведена на отдел (обычный случай — схема заводится на отдел
// один раз, а не на каждого), получал пустой набор правил и нули во всём
// отчёте.
//
...
// общий механизм расчётного модуля. AccountingCacheFreshness — с Фазы 5
// docs/service-shop-boundary-violations-fix собственная независимая копия
// shop (см. domain/services/accounting-cache-freshness.ts), а не
...
export class ResolveShopEmployeeSalaryRulesService {
    ...
    > {
        ...
        // Фаза 3) — зеркало ResolveEmployeeSalaryRulesService.forAllTargets
        // (domains/service), см. WHY там.
