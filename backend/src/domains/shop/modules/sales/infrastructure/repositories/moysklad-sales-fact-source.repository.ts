...
// - **Отдел отгрузки** захардкожен как SHOP_DEPARTMENT_ID — в компании
//   пока существует только один отдел магазина (BitrixDepartment.id = 160),
//   поэтому резолвинг через offlineManagerId → MoySkladEmployee →
//   bitrixEmployee → departmentId не нужен: он лишь отфильтровывал
//   позиции с несопоставленным сотрудником, а деления на отделы всё равно
//   нет. Если в компании появится больше одного отдела магазина —
//   вернуть резолвинг через сотрудника (см. историю этого файла) и снова
//   пропускать позиции без сопоставленного отдела.
