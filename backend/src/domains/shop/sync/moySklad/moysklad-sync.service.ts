...
import { extractIdFromHref, topoSortFolders, ONLINE_MANAGER_ATTR_ID } from './moysklad-sync.mappers';
...
export class MoySkladSyncService {
  ...
  private async uploadDemand(demand: Demand) {
    ...
    const onlineManagerId =
      onlineManagerAttr?.type === 'employee' && onlineManagerAttr.value != null
        ? extractIdFromHref(
            (onlineManagerAttr.value as { meta: { href: string } }).meta.href,
          )
        : null;
    ...
    const salesChannelId = demand.salesChannel
      ...
      : null;
    const stateId = demand.state ? extractIdFromHref(demand.state.meta.href) : null;
    ...
    await this.db.$transaction(async (tx) => {
      ...
      const validPositions = positions.filter((p) => p.assortment != null);
        ...
        const existingProductIds = new Set(
          (
            await tx.moySkladProduct.findMany({
              where: { id: { in: productPositions.map((p) => p.assortment!.id) } },
              select: { id: true },
            })
          ).map((p) => p.id),
        );
          ...
          await tx.moySkladProduct.createMany({
            data: missingProducts.map((p) => {
              ...
              return {
                ...
                salePrice: 0,
                buyPrice: 0,
                folderId: parentId ? (parentFolderById.get(parentId) ?? null) : null,
                archived: false,
                updatedAt: new Date(),
              };
            }),
        ...
        const existingServiceIds = new Set(
          (
            await tx.moySkladService.findMany({
              where: { id: { in: servicePositions.map((p) => p.assortment!.id) } },
              select: { id: true },
            })
          ).map((s) => s.id),
        );
      ...
      const rows = validPositions
        .filter((p) => {
          ...
          return type === 'product' || type === 'variant' || type === 'service';
        })
        .map((p) => {
          ...
          return {
            ...
            demandId: demand.id,
            productId:
              assortmentType === 'product' || assortmentType === 'variant'
                ? p.assortment!.id
                : null,
            serviceId: assortmentType === 'service' ? p.assortment!.id : null,
