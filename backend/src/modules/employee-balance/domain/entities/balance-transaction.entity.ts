...
// Форма, минимально нужная forAccruedLine() ниже (id/direction/period/
// employeeId документа + id/ruleId/name/originalAmount/amount/
// adjustmentComment строки) — НЕ импорт конкретной сущности SalaryAccrual/
// SalaryAccrualLine одного из доменов (см. Фаза 6
// docs/service-shop-boundary-violations-fix: SalaryAccrual/SalaryAccrualLine
// раздельно реализованы по доменам, employee-balance остаётся сквозным
// модулем вне domains/service и domains/shop и не должен зависеть от
// конкретного класса ни одного из них). Оба независимых класса —
// SalaryAccrual/SalaryAccrualLine домена service и ShopSalaryAccrual/
// ShopSalaryAccrualLine домена shop — структурно совместимы с этими
// интерфейсами (у обоих есть такие геттеры), поэтому forAccruedLine()
// работает с документом/строкой любого направления без прямой зависимости
// ни от одного из доменов.
export interface AccrualForBalanceTransaction {
    readonly id: string;
...
export interface AccrualLineForBalanceTransaction {
    readonly id: string;
    readonly ruleId: string;
    readonly name: string;
    readonly originalAmount: number;
    readonly amount: number;
    readonly adjustmentComment: string | undefined;
}
...
export class BalanceTransaction extends AggregateRoot<BalanceTransactionProps> {
    ...
    static forAccruedLine(
        accrual: AccrualForBalanceTransaction,
        line: AccrualLineForBalanceTransaction,
        accruedBy: number,
    ): BalanceTransaction[] {
        const base = {
