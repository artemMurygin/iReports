...
// resource и action: lowercase-слова через дефис/подчёркивание (например
// `employee-balance:view_own`), разделены двоеточием
    ...
    static create(value: string): PermissionCode {
        if (!value || !PERMISSION_CODE_PATTERN.test(value)) {
            throw new ArgumentInvalidException(
                `Permission-код должен быть в формате "resource:action" (строчные буквы/цифры/дефисы/подчёркивания), получено: "${value}"`,
            );
        }
